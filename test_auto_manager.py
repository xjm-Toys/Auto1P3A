...
class TestAutoManager(unittest.TestCase):
    ...
    def test_simple_solver(self):
        ...
        print(
            "\n".join(
                format_captcha_result(answer, solver(path))
                for path, answer in path2expected.items()
            )
        )
    ...
    def test_solver(self):
        ...
        print(
            "\n".join(
                format_captcha_result(expected, solver(path))
                for path, expected in path2expected.items()
            )
        )

    def test_query_question_bank(self):
        questions = ["回答别人的私信提问还需要消耗我5大米怎么办", "Apollo 11是哪一年登月的？", "下面哪个说法错误？"]
        for q in questions:
            try:
